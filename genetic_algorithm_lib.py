...
class GA:
    def __init__(self, 
        ...
        if initial_population is None:
            ...
            raise ValueError("A 2D list is expected to the initail_population parameter but a {initial_population_ndim}-D list found.".format(initial_population_ndim=np.array(initial_population).ndim))
        else:
            self.initial_population = np.array(initial_population)
            self.population = self.initial_population # A np array holding the initial population.
    ...
    def cal_pop_fitness(self):
        ...
        with Pool() as pool:
            args = [(self.ga_util_obj, sol, sol_idx) for sol_idx, sol in enumerate(self.population)]
    ...
    def run(self):
        ...
        for generation in range(self.num_generations):
            ...
            if generation == self.num_generations - 1:
                ...
                self.top_ten_solutions = fitness_solution_pairs[:min(10,self.sol_per_pop)]
            ...
            if self.crossover_type is None:
                if self.num_offspring <= self.keep_parents:
                    ...
                    offspring_crossover = np.concatenate((parents, self.population[0:(self.num_offspring - parents.shape[0])]))
            else:
    ...
    def plot_result(self, title="Generation vs. Fitness", xlabel="Generation", ylabel="Fitness", show_plot=True):
        ...
        matplotlib.pyplot.savefig(f"./genetic_algo_{self.generations_completed}gens_{self.pop_size[0]}pop.png") 
        if show_plot:
